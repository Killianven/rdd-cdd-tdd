...
const cli = meow({								// sert the format (key,value)
	help: [
		'Usage',
		'  $ weather <input>',					// what we search
		'',
		'Options',												// what are the inputs/parametres
		'  city [Default: Dhaka]',
...
function _toCelcius(temp) {												// convert data to Celcius
	return Math.round(((temp - 32) * 5) / 9);
}

updateNotifier({ pkg}).notify();

weather(cli.input, (err, result) => {										//run the research for the weather
	if (err) {
